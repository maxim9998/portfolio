...
const SoftSkill: FC<ISoftSkillExtended> = ({ icon, name, reverse, description }) => {
  ...
  return (
    <div className={`flex items-center w-full justify-between pt-10 ${renderLeftOrRight.direction}`}>
      <div className={`w-1/2 flex items-center ${renderLeftOrRight.justify} px-6 xs:px-3`}>{icon}</div>
      <div className={`rounded-lg w-1/2 flex flex-col justify-center px-6 xs:px-3 ${renderLeftOrRight.description}`}>
        <h3 className={`text-2xl xs:text-lg text-green ${renderLeftOrRight.text}`}>{name}</h3>
